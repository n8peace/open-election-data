...
async function readList(backend: 'claude-code' | 'codex', county: string) {
  ...
  if (backend === 'claude-code' && !claudePlanAllowed()) return readViaApi(county, listModel());
  try { return parse(await runCli(backend, prompt(county))); } catch (e) {
    ...
    return readViaApi(county, backend === 'codex' ? 'anthropic/claude-haiku-4.5' : listModel());
  }
}
